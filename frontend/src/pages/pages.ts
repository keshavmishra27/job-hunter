...
export async function renderHome() {
  ...
  el.innerHTML = `
    <div style="padding:28px;max-width:980px;margin:0 auto">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:16px;">
        <div>
          <h1 style="margin:0;font-size:28px">Job Hunter</h1>
          <p style="margin:6px 0 18px;color:var(--text-muted)">AI-assisted internship discovery, ranking and outreach — get started in seconds.</p>
          <div style="display:flex;gap:10px;flex-wrap:wrap">
            <button class="btn btn-primary" id="home-open-app">Open App</button>
            <button class="btn btn-secondary" id="home-upload-resume">Upload Resume</button>
            <button class="btn btn-secondary" id="home-check-backend">Check Backend</button>
          </div>
        </div>
        <div style="min-width:220px;text-align:right;color:var(--text-muted)">
          <div style="font-size:13px">Status</div>
          <div id="home-backend-status" style="margin-top:8px">Checking…</div>
        </div>
      </div>

      <div style="margin-top:28px" id="home-features">
        <div class="section-header"><span class="section-title">Quick Tour</span></div>
        <div style="display:flex;gap:12px;margin-top:12px;flex-wrap:wrap"><div class="card" style="flex:1;min-width:220px"><div class="card-title">Fetch & Rank</div><div style="font-size:13px;color:var(--text-muted);margin-top:8px">Pull opportunities from multiple sources and rank them for you.</div></div><div class="card" style="flex:1;min-width:220px"><div class="card-title">Drafts</div><div style="font-size:13px;color:var(--text-muted);margin-top:8px">Generate outreach drafts tailored to each role and review them.</div></div><div class="card" style="flex:1;min-width:220px"><div class="card-title">Repo Intelligence</div><div style="font-size:13px;color:var(--text-muted);margin-top:8px">Analyze your GitHub to surface matching projects for a role.</div></div></div>
      </div>
    </div>`;
...
export async function renderDashboard() {
  ...
  try {
    ...
    el.innerHTML = `
      <div class="stats-grid"><div class="stat-card"><div class="stat-number">${stats.total_jobs_fetched ?? 0}</div><div class="stat-label">Jobs Fetched</div></div><div class="stat-card"><div class="stat-number">${stats.matched_for_user ?? 0}</div><div class="stat-label">Matched & Ranked</div></div><div class="stat-card"><div class="stat-number">${stats.drafts_pending_review ?? 0}</div><div class="stat-label">Drafts to Review</div></div><div class="stat-card"><div class="stat-number">${stats.drafts_approved ?? 0}</div><div class="stat-label">Approved Drafts</div></div><div class="stat-card"><div class="stat-number">${stats.emails_sent ?? 0}</div><div class="stat-label">Emails Sent</div></div><div class="stat-card" style="border-left:3px solid var(--accent)"><div class="stat-number">${stats.freelance_gigs ?? 0}</div><div class="stat-label">Freelance Gigs</div></div></div><div class="section-header"><span class="section-title">Quick Actions</span></div><div style="display:flex;gap:12px;flex-wrap:wrap"><button class="btn btn-primary" id="dash-fetch-btn">Fetch & Rank Jobs</button><button class="btn btn-secondary" id="dash-drafts-btn">Review Drafts</button><button class="btn btn-secondary" id="dash-freelance-btn">Browse Freelancing</button></div>`;
...
export async function renderProfile() {
  ...
  el.innerHTML = `
    <div class="two-col"><div><div class="section-header"><span class="section-title">Upload Resume</span></div><div class="card"><div class="upload-zone" id="upload-zone"><div class="upload-icon"></div><div class="upload-text">Drop your PDF resume here</div><div class="upload-sub">or click to browse</div><input type="file" id="resume-file-input" accept=".pdf" style="display:none" /></div><div style="margin-top:16px" id="upload-status"></div></div><!--  Settings Card --><div class="section-header" style="margin-top:24px"><span class="section-title">Profile Settings</span></div><div class="card" id="settings-card"><div class="form-group"><label class="card-title" for="grad-year-input">Graduation Year</label><div style="font-size:12px;color:var(--text-muted);margin-bottom:6px">e.g. 2028 — used to detect if you're eligible for internships targeted at specific batches
            </div><input class="form-input" id="grad-year-input" type="number" min="2024" max="2032"
              placeholder="e.g. 2028" style="width:160px" /></div><div class="form-group" style="margin-top:16px"><label class="card-title" for="tg-chat-id-input">✈️ Telegram Chat ID</label><div style="font-size:12px;color:var(--text-muted);margin-bottom:6px">1. Create a bot via <a href="https://t.me/BotFather" target="_blank" style="color:var(--accent-light)">@BotFather</a>and add <code>TELEGRAM_BOT_TOKEN</code>to <code>.env</code><br/>2. Message your bot, then visit <code>https://api.telegram.org/bot&lt;TOKEN&gt;/getUpdates</code>to find your chat ID
            </div><input class="form-input" id="tg-chat-id-input" type="text"
              placeholder="e.g. 123456789 or @yourchannel" style="width:100%;max-width:320px" /></div><div style="display:flex;gap:10px;margin-top:16px;flex-wrap:wrap"><button class="btn btn-primary btn-sm" id="save-settings-btn">💾 Save Settings</button><button class="btn btn-secondary btn-sm" id="send-telegram-btn">✈️ Send Eligible to Telegram</button></div><div id="settings-status" style="margin-top:10px;font-size:13px"></div></div></div><div><div class="section-header"><span class="section-title">Current Profile</span></div><div class="card" id="profile-card"><div class="empty-state" style="padding:32px"><div class="empty-icon"></div><div class="empty-title">No profile yet</div><div class="empty-sub">Upload and parse your resume to build your profile.</div></div></div></div></div>`;
...
export async function renderJobs() {
  ...
  const sourceGroups = [
    { id: "internship", label: " Internship Boards" },
    { id: "startup", label: " Startup Boards" },
    { id: "remote", label: "🌍 Remote Boards" },
  ];
  let selected = new Set(["internship", "startup"]);
  let showApplied = false;
  let fetchAttempted = false;
  let selectedCompLevels = ["low", "medium", "high"];

  el.innerHTML = `
    <div class="section-header"><span class="section-title">Fetch & Rank Opportunities</span></div><div class="card" style="margin-bottom:24px"><div class="card-title" style="margin-bottom:10px">Select Source Groups</div><div class="source-chips">${sourceGroups.map((sg) =>`<button class="source-chip ${selected.has(sg.id) ? "selected" : ""}" data-source-group="${sg.id}">${sg.label}</button>`).join("")}
      </div><div style="margin-top: 12px; display: flex; gap: 12px; align-items: center;">
        <select id="competition-filter" class="form-input" style="width: auto; padding: 6px 12px; height: 36px;">
          <option value="all">All Competition Levels</option>
          <option value="low">Low Competition Only</option>
          <option value="low,medium">Low & Medium</option>
        </select>
        <button class="btn btn-primary" id="fetch-btn">Fetch & Rank</button><span id="fetch-status" style="font-size:13px;color:var(--text-secondary);margin-left:12px"></span>
      </div></div><div style="margin-bottom: 12px;"><label style="font-size:13px; cursor:pointer;"><input type="checkbox" id="show-applied-toggle" />Show Applied/Tracked</label></div><div id="jobs-list" class="job-list"></div>`;

  el.querySelectorAll(".source-chip").forEach((chip) =>{
    chip.addEventListener("click", () =>{
      const sg = (chip as HTMLElement).dataset.sourceGroup!;
      if (selected.has(sg)) selected.delete(sg);
      else selected.add(sg);
      chip.classList.toggle("selected", selected.has(sg));
      loadJobs();
    });
  });
  ...
  document.getElementById("competition-filter")?.addEventListener("change", (e) => {
    ...
    if (val === "all") selectedCompLevels = ["low", "medium", "high"];
    else if (val === "low") selectedCompLevels = ["low"];
    else if (val === "low,medium") selectedCompLevels = ["low", "medium"];
    loadJobs(false);
  });
  ...
  document.getElementById("fetch-btn")?.addEventListener("click", async () =>{
    ...
    if (selected.size === 0) {
        toast("Please select at least one source group.", "error");
        return;
    }
    btn.disabled = true;
    ...
    try {
      const res = await api.fetchOpportunities(USER_ID, [...selected], true, true);
      fetchAttempted = true;
      toast(`Fetched ${res.fetched} items. Found ${res.ranked} new opportunities.`, "success");
      status.textContent = ` ${res.ranked} new added`;
      loadJobs();
    } catch (e: any) {
      toast(e.message, "error");
    } finally {
  ...
  async function loadJobs(retryOnStale = true) {
    ...
    list.innerHTML = `<div style="display:flex;gap:8px;align-items:center"><div class="spinner"></div>Loading ranked opportunities…</div>`;
    try {
      let jobs = await api.getRankedOpportunities(USER_ID, 30, [...selected]);
      
      if (!showApplied) {
         jobs = jobs.filter(j =>j.status !== 'applied' && j.status !== 'drafted');
      }
      
      jobs = jobs.filter(j => {
         const comp = j.competition_label || "medium";
         return selectedCompLevels.includes(comp);
      });

      if (retryOnStale && fetchAttempted && jobs.length === 0) {
        list.innerHTML = `<div style="display:flex;gap:8px;align-items:center"><div class="spinner"></div>Fetching new opportunities…</div>`;
        try {
          const res = await api.fetchOpportunities(USER_ID, [...selected], false, true);
          toast(`Fetched ${res.ranked} opportunities.`, "success");
          fetchAttempted = true;
      ...
      if (!jobs.length) {
        list.innerHTML = `<div class="empty-state"><div class="empty-icon"></div><div class="empty-title">No opportunities yet</div><div class="empty-sub">Click "Fetch & Rank" to pull opportunities.</div></div>`;
        return;
      }

      list.innerHTML = jobs.map((j: any) =>{
        const isApplied = j.status === 'applied' || j.status === 'drafted';
        const compLabel = j.competition_label || 'medium';
        return `
        <div class="job-card glass-panel" data-job-id="${j.id}" style="border-left: 3px solid var(--accent); position: relative; overflow: hidden;"><div class="job-company-logo" style="box-shadow: 0 0 15px var(--accent-glow);">${(j.organization || j.company || "?")[0].toUpperCase()}</div><div class="job-info"><div class="job-title" style="font-size: 16px; font-weight: 700; color: #fff;">${j.title} ${isApplied ? '<span class="badge badge-applied" style="background: rgba(16, 185, 129, 0.15); color: #34d399; border: 1px solid rgba(16, 185, 129, 0.3);">✓ Tracked</span>' : ''}</div><div class="job-meta" style="margin-top: 6px;"><span style="color: var(--text-secondary); font-weight: 500;">${j.organization || j.company || "Unknown"}</span><span style="color: var(--text-muted);">&bull;</span><span>${j.location || "—"}</span><span>${modeChip(j.mode || "offline")}</span><span style="color:var(--text-muted);font-size:11px; padding: 2px 6px; background: rgba(255,255,255,0.05); border-radius: 4px;">${j.source} (${j.source_group || ''})</span></div></div><div style="display:flex;flex-direction:column;align-items:flex-end;gap:10px"><div class="job-score" style="font-size: 14px; font-family: var(--font-display); box-shadow: 0 0 10px var(--accent-glow);">${scorePct(j.score || 0)} Fit <span class="competition-badge competition-${compLabel}" style="margin-left: 6px;">${compLabel.toUpperCase()} COMP</span></div><div style="display:flex;gap:6px"><button class="btn btn-secondary btn-sm view-job-btn" data-job-id="${j.id}" data-job='${JSON.stringify(j).replace(/'/g, "&#39;")}'>Intelligence</button><button class="btn btn-primary btn-sm gen-draft-btn" data-job-id="${j.id}">Queue Autopilot ⚡</button>${!isApplied ? `<button class="btn btn-success btn-sm mark-applied-btn" data-job-id="${j.id}">✓ Applied</button>` : ''}<button class="btn btn-danger btn-sm remove-job-btn" data-job-id="${j.id}">Hide</button>
            </div></div></div>`}).join("");
      ...
      list.querySelectorAll(".gen-draft-btn").forEach((btn) =>{
        btn.addEventListener("click", async () =>{
          ...
          try {
            ...
            await api.updateOpportunityStatus(jobId, USER_ID, 'drafted');
            loadJobs();
          } catch (e: any) {
            toast(e.message, "error");
          } finally {
      ...
      list.querySelectorAll(".mark-applied-btn").forEach((btn) =>{
        btn.addEventListener("click", async () =>{
          ...
          (btn as HTMLButtonElement).disabled = true;
          try {
            await api.updateOpportunityStatus(jobId, USER_ID, 'applied');
            toast("Marked as applied! Opportunity tracked.", "success");
            loadJobs();
          } catch (e: any) {
            toast(e.message, "error");
      ...
      list.querySelectorAll(".remove-job-btn").forEach((btn) =>{
        btn.addEventListener("click", async () =>{
          const jobId = (btn as HTMLElement).dataset.jobId!;
          const ok = confirm("Remove this opportunity from the list? You can always fetch again later.");
          if (!ok) return;
          (btn as HTMLButtonElement).disabled = true;
          try {
            await api.updateOpportunityStatus(jobId, USER_ID, 'dismissed');
  ...
  function showJobModal(j: any) {
    ...
    const skillTags = matchedSkills.length
      ? `<div style="margin-top:6px;display:flex;flex-wrap:wrap;gap:6px">${matchedSkills.map((s: string) =>`<span class="tag" style="background:rgba(99,102,241,0.18);color:#a5b4fc">${s}</span>`).join("")}
        </div>`
    ...
    const projectTags = matchedProjects.length
      ? `<div style="margin-top:6px;display:flex;flex-wrap:wrap;gap:6px">${matchedProjects.map((p: string) =>`<span class="tag" style="background:rgba(16,185,129,0.15);color:#6ee7b7;font-size:11px">${p}</span>`).join("")}
        </div>`
      : `<span style="font-size:12px;color:var(--text-muted)">No project keywords matched</span>`;

    openModal(`
      <div class="modal-title">${j.title}</div><div style="margin-bottom:16px"><span style="font-size:15px;color:var(--text-secondary)">${j.company}</span><span style="margin-left:12px">${modeChip(j.mode || "offline")}</span></div><div class="card-title">Score Breakdown</div><div class="score-breakdown" style="margin-bottom:16px">${Object.entries(breakdown).map(([k, v]) =>`
      ...
      </div><div style="margin-bottom:12px"><div class="card-title" style="margin-bottom:4px">Matched Skills (${matchedSkills.length})</div>${skillTags}
      </div><div style="margin-bottom:20px"><div class="card-title" style="margin-bottom:4px">Matched Projects (${matchedProjects.length})</div>${projectTags}
      </div>${j.apply_link ? `<a href="${j.apply_link}" target="_blank" class="btn btn-primary">Apply →</a>` : ""}
    `);
  }
...
export async function renderInternships() {
  ...
  function renderList(notices: any[]) {
    ...
    list.innerHTML = filtered.map((n: any) =>`
      <div class="intern-card glass-panel" data-id="${n.notice_id}" style="border-left: 3px solid #8a2be2; position: relative; overflow: hidden;"><div class="intern-card-left"><div class="intern-logo" style="box-shadow: 0 0 15px rgba(138, 43, 226, 0.4);">${(n.company || "?")[0].toUpperCase()}</div></div><div class="intern-card-body"><div class="intern-card-top"><span class="intern-title" style="font-size: 16px; font-weight: 700; color: #fff;">${n.title || "Untitled"}</span>${eligBadge(n.eligibility_status || "unknown")}
          ...
          <div class="intern-actions" style="display:flex;gap:6px"><button class="btn btn-secondary btn-sm view-notice-btn" data-id="${n.notice_id}">Intelligence</button>${n.apply_link ? `<a class="btn btn-success btn-sm" href="${n.apply_link}" target="_blank">Apply ↗</a>` : ""}
            <button class="btn btn-sm save-notice-btn ${n.status === "saved" ? "btn-success" : "btn-primary"}"
              data-id="${n.notice_id}">${n.status === "saved" ? "✓ Tracked" : "Queue Autopilot ⚡"}</button></div></div></div>`).join("");
  ...
  // ── Initial render ──────────────────────────────────────────────────────────
  el.innerHTML = `
    <div class="section-header"><span class="section-title">📡 Internship News Scraper</span><span style="font-size:12px;color:var(--text-muted)">Detect · Score · Track</span></div><!-- Source Registry --><div class="card" style="margin-bottom:20px"><div class="intern-src-header"><div class="card-title" style="margin:0">Source Registry</div><div class="intern-src-tabs"><button class="intern-src-tab active" id="srctab-website" data-srctab="website">🌐 Website</button><button class="intern-src-tab" id="srctab-telegram" data-srctab="telegram">✈️ Telegram</button><button class="intern-src-tab" id="srctab-gmail" data-srctab="gmail">📧 Gmail</button></div></div><div id="src-website-chips" class="source-chips" style="margin-top:12px">${WEB_SOURCES.map(s =>`
          <button class="source-chip ${webSelected.has(s.id) ? "selected" : ""}" data-source="${s.id}" data-group="web">${s.icon} ${s.label}
          </button>`).join("")}
      </div><div id="src-telegram-chips" class="source-chips" style="margin-top:12px;display:none">${TG_SOURCES.map(s =>`
          <button class="source-chip ${tgSelected.has(s.id) ? "selected" : ""}" data-source="${s.id}" data-group="tg">${s.icon} ${s.label}
            <span style="font-size:10px;color:var(--text-muted);margin-left:4px">5 channels</span></button>`).join("")}
        <div style="margin-top:8px;font-size:12px;color:var(--text-muted)">Scrapes: @JobsAndInternshipsIndia · @internshipsalert · @internship_update · @HiringIndia · @TechJobsIndia
        </div></div><div id="src-gmail-chips" class="source-chips" style="margin-top:12px;display:none">${GMAIL_SOURCES.map(s =>`
          <button class="source-chip ${gmailSelected.has(s.id) ? "selected" : ""}" data-source="${s.id}" data-group="gmail">${s.icon} ${s.label}
          </button>`).join("")}
        <div id="gmail-status-line" style="margin-top:8px;font-size:12px;color:var(--text-muted)">Checking connection…
        </div></div><div style="display:flex;align-items:center;gap:12px;margin-top:16px"><button class="btn btn-primary" id="intern-fetch-btn">⚡ Fetch &amp; Process</button><span id="intern-fetch-status" style="font-size:13px;color:var(--text-secondary)"></span></div></div><!-- Pipeline status (hidden until fetch) --><div id="intern-pipeline-wrap" style="display:none;margin-bottom:20px">${pipelineHTML(-1)}
    </div><!-- Filter tabs --><div class="intern-tabs"><button class="intern-tab active" id="intern-tab-all" data-tab="all">All (0)</button><button class="intern-tab" id="intern-tab-eligible" data-tab="eligible">Eligible (0)</button><button class="intern-tab" id="intern-tab-saved" data-tab="saved">Saved (0)</button><button class="intern-tab" id="intern-tab-applied" data-tab="applied">Applied (0)</button></div><!-- Feed --><div id="intern-feed" class="intern-feed"></div>`;
...
export async function renderDrafts() {
  ...
  try {
    ...
    el.innerHTML = `
      <div class="section-header"><span class="section-title">Autopilot Queue (${drafts.length})</span></div><div class="job-list" id="draft-list"></div>`;

    const list = document.getElementById("draft-list")!;
    list.innerHTML = drafts.map((d: any) =>`
      <div class="draft-card glass-panel" id="draft-${d.draft_id}" style="border-left: 3px solid #00f0ff; position: relative; overflow: hidden; padding: 16px;"><div class="draft-header" style="display:flex; justify-content:space-between; align-items:flex-start;"><div><div class="draft-subject" style="font-size: 16px; font-weight: 700; color: #fff;">${d.subject || "(no subject)"}</div><div style="font-size:13px;color:var(--text-secondary);margin-top:6px;font-weight:500;">${d.company} <span style="color:var(--text-muted)">&bull;</span> ${d.job_title}</div></div><div style="display:flex;align-items:center;gap:12px">${statusChip(d.status)}
            <div class="draft-actions" style="display:flex; gap:6px;"><button class="btn btn-secondary btn-sm view-draft-btn" data-draft-id="${d.draft_id}">Review</button>${d.status !== "approved" && d.status !== "sent" ? `<button class="btn btn-success btn-sm approve-btn" data-draft-id="${d.draft_id}">Approve</button>` : ""}
              ${d.status === "approved" ? `<button class="btn btn-primary btn-sm send-btn" data-draft-id="${d.draft_id}">Send Outreach 🚀</button>` : ""}
...
export async function renderRepos() {
  ...
  el.innerHTML = `<div class="repo-page-wrapper"><div class="repo-connect-panel" id="repo-connect-panel"><div class="spinner"></div><span style="margin-left:10px">Checking GitHub connection…</span></div><div id="repo-main-content" style="display:none"><div class="repo-toolbar"><div class="repo-toolbar-left"><div class="repo-github-badge" id="repo-github-badge"></div><div class="role-selector-wrap"><label class="role-selector-label">Target Role</label><select class="role-select" id="role-select"></select></div></div><div class="repo-toolbar-right"><button class="btn btn-secondary btn-sm" id="repo-sync-btn">⟳ Sync Repos</button><button class="btn btn-primary" id="repo-analyze-btn">⚡ Analyze &amp; Rank</button></div></div><div id="repo-status-bar" class="repo-status-bar" style="display:none"></div><div id="top5-section" style="display:none"><div class="section-header" style="margin-bottom:20px"><span class="section-title">Top 5 Repositories</span><span class="repo-role-pill" id="active-role-pill"></span></div><div class="repo-top5-grid" id="repo-top5-grid"></div></div><div id="all-repos-section" style="display:none"><div class="section-header" style="margin-top:40px;margin-bottom:16px"><span class="section-title" id="all-repos-count-title">All Repositories</span><button class="btn btn-secondary btn-sm" id="toggle-all-repos">Show All</button></div><div class="job-list" id="all-repos-list" style="display:none"></div></div></div></div>`;
...
function renderRepoCard(repo: any, rank: number): string {
  ...
  return `
  <div class="repo-card glass-panel" data-repo-id="${repo.repo_id}" title="Click to expand full analysis"><div class="repo-card-header"><div class="repo-card-title-row"><div class="repo-card-rank" style="color:${rankColor}; text-shadow: 0 0 10px ${rankColor}88;">#${rank}</div><div class="repo-card-name">${repo.name}</div></div><div class="repo-card-score-circle" style="--score-color:${rankColor}"><svg viewBox="0 0 48 48"><circle cx="24" cy="24" r="20" fill="none" stroke="var(--border)" stroke-width="3"/><circle cx="24" cy="24" r="20" fill="none" stroke="${rankColor}" stroke-width="3"
            stroke-dasharray="${((finalPct/100)*125.66).toFixed(1)} 125.66"
            stroke-linecap="round" transform="rotate(-90 24 24)"
            style="filter: drop-shadow(0 0 4px ${rankColor}88);"/><text x="24" y="28" text-anchor="middle" font-size="10" font-weight="800" fill="#fff">${repo.final_score.toFixed(1)}</text></svg></div></div><div class="repo-card-desc" style="color:var(--text-secondary); margin-bottom: 12px; font-size: 13px;">${repo.description || "<em style='opacity:0.5'>No description</em>"}</div><div class="repo-card-meta" style="display:flex; gap:12px; font-size:12px; color:var(--text-muted); margin-bottom:12px;">${repo.language ? `<span style="display:flex; align-items:center; gap:4px;"><span class="repo-lang-dot" style="background:${lc}; width:8px; height:8px; border-radius:50%; box-shadow: 0 0 8px ${lc};"></span>${repo.language}</span>` : ""}
      ${repo.stars > 0 ? `<span>★ ${repo.stars}</span>` : ""}
      ${repo.forks > 0 ? `<span>⑂ ${repo.forks}</span>` : ""}
    </div><div class="repo-badges-row" style="display:flex; gap:6px; flex-wrap:wrap; margin-bottom:16px;">${badges}</div><div class="repo-card-scores" style="display:flex; justify-content:space-between; background: rgba(0,0,0,0.2); border-radius:var(--radius-sm); padding:12px;">${renderScoreRing(repo.uniqueness_score,     "Unique",  "#a78bfa")}
    ...
    </div>${repo.selection_reason ? `<div class="repo-card-reason" style="margin-top:12px; font-size:12px; color:var(--text-secondary); border-left: 2px solid ${rankColor}; padding-left:8px;">${repo.selection_reason}</div>` : ""}
    <div class="repo-card-footer" style="margin-top:16px; display:flex; justify-content:space-between; align-items:center;"><a class="repo-card-link" href="${repo.html_url}" target="_blank" onclick="event.stopPropagation()" style="color:var(--accent); font-weight:600; font-size:13px; display:flex; align-items:center; gap:4px;">View on GitHub ↗</a><span class="repo-expand-hint" style="color:var(--text-muted); font-size:11px;">Click to expand</span></div></div>`;
}

function renderRepoListRow(repo: any, rank: number): string {
  return `
  <div class="job-card repo-list-row" data-repo-id="${repo.repo_id}" style="cursor:pointer;"><div class="job-company-logo" style="font-size:14px;">${repo.name[0].toUpperCase()}
    </div><div class="job-info"><div class="job-title">#${rank} ${repo.name}</div><div class="job-meta"><span>${repo.language || "—"}</span><span>★ ${repo.stars}</span>${repo.description ? `<span style="max-width:300px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${repo.description}</span>` : ""}
      </div></div><div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px"><div class="job-score">${repo.final_score.toFixed(1)} / 10</div><div style="font-size:11px;color:var(--text-muted)">${repo.analyzed ? "analysed" : "not analysed"}</div></div></div>`;
}
...
export async function renderSent() {
  ...
  try {
    ...
    el.innerHTML = `
      <div class="section-header"><span class="section-title">Sent Log (${log.length})</span></div><div class="job-list">${log.map((s: any) =>`
          <div class="draft-card"><div class="draft-header"><div><div class="draft-subject">${s.subject || "—"}</div><div style="font-size:13px;color:var(--text-secondary);margin-top:4px">${s.recipient} · ${s.sent_at ? new Date(s.sent_at).toLocaleString() : "—"}
                </div></div>${statusChip(s.status)}
            </div></div>`).join("")}
      </div>`;
  } catch (e: any) {
    el.innerHTML = `<div class="empty-state"><div class="empty-icon"></div><div class="empty-title">${e.message}</div></div>`;
...
export async function renderApplied() {
  ...
  try {
    ...
    if (!apps.length) {
      el.innerHTML = `<div class="empty-state"><div class="empty-title">No applications yet</div><div class="empty-sub" style="margin-top: 16px;">Mark jobs as applied from the Ranked Jobs page.</div></div>`;
      return;
    }
    el.innerHTML = `
      <div class="section-header"><span class="section-title">Tracker (${apps.length})</span></div><div class="job-list" id="applied-list"></div>`;
    const list = document.getElementById("applied-list")!;
    list.innerHTML = apps.map((a: any) =>`
      <div class="job-card glass-panel applied-row" id="app-${a.application_id}" style="border-left: 3px solid #10b981; position: relative; overflow: hidden; padding: 16px;"><div class="job-company-logo" style="box-shadow: 0 0 15px rgba(16, 185, 129, 0.4);">${(a.company_name || "?")[0].toUpperCase()}</div><div class="job-info"><div class="job-title" style="font-size: 16px; font-weight: 700; color: #fff;">${a.role_title || "\u2014"}</div><div class="job-meta" style="margin-top: 6px;"><span style="color: var(--text-secondary); font-weight: 500;">${a.company_name || "\u2014"}</span><span style="color: var(--text-muted);">&bull;</span><span>${a.source || "\u2014"}</span><span style="color: var(--text-muted);">&bull;</span><span>${a.location || "\u2014"}</span><span style="color: var(--text-muted);">&bull;</span><span style="color:var(--text-muted);font-size:11px; padding: 2px 6px; background: rgba(255,255,255,0.05); border-radius: 4px;">Applied: ${a.applied_at ? new Date(a.applied_at).toLocaleDateString() : "\u2014"}</span></div>${a.follow_up_date ? `<div style="font-size:11px; color: #f59e0b; margin-top:8px; font-weight: 600;">\u23f0 FOLLOW-UP: ${new Date(a.follow_up_date).toLocaleDateString()}</div>` : ""}
          ${a.draft_subject ? `<div style="font-size:11px; color: var(--text-muted); margin-top:4px;">\u2709\ufe0f DRAFT: ${a.draft_subject}</div>` : ""}
        </div><div style="display:flex;flex-direction:column;align-items:flex-end;gap:12px"><span class="badge ${STATUS_COLORS[a.status] || "badge-new"}" style="font-size: 11px; font-weight: 700; letter-spacing: 0.5px;">${a.status}</span><div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end"><button class="btn btn-secondary btn-sm set-followup-btn" data-app-id="${a.application_id}" data-follow-up="${a.follow_up_date || ""}">Set Follow-up</button><button class="btn btn-danger btn-sm unmark-btn" data-app-id="${a.application_id}">Remove</button></div></div></div>`).join("");
    ...
    list.querySelectorAll(".set-followup-btn").forEach((btn) =>{
      btn.addEventListener("click", () =>{
        ...
        openModal(`
          <div class="modal-title">Set Follow-up Date</div><div class="form-group"><label class="form-label">Date</label><input class="form-input" type="date" id="followup-date" value="${existing ? existing.split("T")[0] : ""}" /></div><div style="display:flex;gap:8px;margin-top:8px"><button class="btn btn-primary" id="save-followup-btn" data-app-id="${appId}">Save</button><button class="btn btn-secondary" onclick="document.getElementById('modal-overlay').classList.add('hidden')">Cancel</button></div>`);
...
export async function renderFreelancing() {
  ...
  // ── Render the page ────────────────────────────────────────────────────
  el.innerHTML = `
    <!-- Source Chips --><div style="margin-bottom:16px"><div style="font-size:13px;font-weight:600;color:var(--text-secondary);margin-bottom:8px">Freelance Sources</div><div class="source-chips" id="fl-source-chips">${FREELANCE_SOURCES.map(s =>`
          <button class="source-chip ${flSelected.has(s.id) ? "selected" : ""}" data-source="${s.id}">${s.icon} ${s.label}
          </button>`).join("")}
      </div><div style="display:flex;align-items:center;gap:12px;margin-top:12px"><button class="btn btn-primary" id="fl-fetch-btn">⚡ Fetch Gigs</button><span id="fl-fetch-status" style="font-size:13px;color:var(--text-secondary)"></span></div></div><!-- Pipeline --><div id="fl-pipeline-wrap" style="display:none;margin-bottom:16px">${flPipelineHTML(-1)}
    </div><!-- Filter Tabs --><div class="fl-tabs"><button class="fl-tab active" id="fl-tab-all" data-tab="all">All (0)</button><button class="fl-tab" id="fl-tab-saved" data-tab="saved">Saved (0)</button><button class="fl-tab" id="fl-tab-applied" data-tab="applied">Applied (0)</button><button class="fl-tab" id="fl-tab-progress" data-tab="in_progress">In Progress (0)</button></div><!-- Feed --><div id="fl-feed"></div>`;
